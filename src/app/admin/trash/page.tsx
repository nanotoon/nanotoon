...
export default function AdminTrashPage() {
  ...
  useEffect(() => {
    ...
    let c = false
    async function load() {
      const { data } = await anonDb.from('series').select('*, profiles!series_author_id_fkey(display_name, handle)').eq('is_removed', true).order('removed_at', { ascending: false }) as any
      if (!c) {
        setRemovedSeries(data ?? [])
        setLoading(false)
      }
    }
          ...
          {removedSeries.map(s => (
            <div key={s.id} className="bg-[#18181b] rounded-xl p-3 border border-[#27272a] flex items-center gap-3">
              ...
              <div className="flex gap-1.5 shrink-0">
                ...
                <button onClick={() => permanentDelete(s.id, s.title)} className="px-3 py-1.5 border border-red-500/30 rounded-lg text-xs text-[#f87171] cursor-pointer hover:bg-red-500/10 bg-transparent">Delete Forever</button>
              </div>
            </div>
          ))}
        </div>
