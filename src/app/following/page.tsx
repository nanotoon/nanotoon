...
export default function FollowingPage() {
  ...
  useEffect(() => {
    ...
    let c = false
    supabase.from('follows').select('*, profiles!follows_following_id_fkey(id, display_name, handle, avatar_url)')
      .eq('follower_id', user.id).order('created_at', { ascending: false })
      .then(({ data }: any) => { if (!c) { setFollowing(data ?? []); setLoading(false) } })
    return () => { c = true }
  }, [user, supabase])
  ...
  return (
    <div className="px-4 md:px-8 py-6">
      ...
      {loading ? <p className="text-center py-6 text-[#52525b] text-sm">Loading...</p> : following.length === 0 ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-2 md:gap-3 mb-4">
          {following.map(f => { const p = f.profiles; if (!p) return null; return (
            <div key={f.following_id} className="bg-[#18181b] rounded-xl md:rounded-2xl p-2 md:p-3 flex items-center gap-2 md:gap-2.5 border border-[#27272a]">
              {p.avatar_url ? <img src={p.avatar_url} className="w-7 h-7 md:w-9 md:h-9 rounded-full object-cover" /> : <Avatar name={p.display_name} size={36} className="w-7 h-7 md:w-9 md:h-9" />}
              ...
              <button onClick={() => unfollow(f.following_id, p.display_name)}
                ...
                <span className="hidden md:inline">Following</span><span className="md:hidden">✕</span>
              </button>
            </div>
          )})}
        </div>
