          ...
          <div className="relative">
            ...
            {imgs.length>1 && <div className="flex flex-col items-center gap-2 mt-3">
              <div className="flex items-center justify-center gap-3">
                {!(isRTL ? page===imgs.length-1 : page===0) && <button onClick={()=>setPage(p=> isRTL ? Math.min(imgs.length-1,p+1) : Math.max(0,p-1))} className="w-[40px] h-[38px] border border-[#3f3f46] rounded-xl text-sm cursor-pointer bg-transparent flex items-center justify-center text-[#a1a1aa] hover:border-[#a855f7]">◀</button>}
                {(isRTL ? page===imgs.length-1 : page===0) && <div className="w-[40px] h-[38px]" />}
                ...
                {!(isRTL ? page===0 : page===imgs.length-1) && <button onClick={()=>setPage(p=> isRTL ? Math.max(0,p-1) : Math.min(imgs.length-1,p+1))} className="w-[40px] h-[38px] border border-[#3f3f46] rounded-xl text-sm cursor-pointer bg-transparent flex items-center justify-center text-[#a1a1aa] hover:border-[#a855f7]">▶</button>}
                {(isRTL ? page===0 : page===imgs.length-1) && <div className="w-[40px] h-[38px]" />}
              </div>
      ...
      {showFullscreen && imgs.length > 0 && (
        <div className="fixed inset-0 bg-black z-[300] flex flex-col"
          ...
          <div className="flex-1 flex items-center justify-center overflow-hidden px-2 relative">
            ...
            <img src={imgs[fullscreenPage]} className="h-full max-w-full object-contain select-none" alt={item.title} draggable={false} onClick={e=>e.stopPropagation()} />
