...
export default function GalleryDetailPage() {
  ...
  const [editReadingMode, setEditReadingMode] = useState<'horizontal' | 'webtoon'>('horizontal')
  const [editImages, setEditImages] = useState<string[]>([])
  const [editSaving, setEditSaving] = useState(false)
  // Comment edit/reply state
  ...
  async function editCommentSave(commentId: string) {
    if (!editCommentText.trim()) return
    const { error } = await supabase.from('gallery_comments').update({ body: editCommentText.trim() }).eq('id', commentId)
    if (error) { show('Edit failed: ' + error.message); return }
    setComments(prev => prev.map(c => c.id === commentId ? { ...c, body: editCommentText.trim() } : c))
    setEditingCommentId(null); setEditCommentText(''); show('Comment edited!')
  }
  ...
  function openEditModal() {
    ...
    setEditReadingMode(item.reading_mode || 'horizontal')
    setEditImages([...(item.image_urls || [])])
    setEditing(true)
  }

  function moveEditImage(fromIdx: number, direction: 'up' | 'down') {
    const toIdx = direction === 'up' ? fromIdx - 1 : fromIdx + 1
    if (toIdx < 0 || toIdx >= editImages.length) return
    const newImgs = [...editImages]
    const temp = newImgs[fromIdx]
    newImgs[fromIdx] = newImgs[toIdx]
    newImgs[toIdx] = temp
    setEditImages(newImgs)
  }

  function removeEditImage(idx: number) {
    if (editImages.length <= 1) { show('Must have at least 1 image'); return }
    if (!confirm(`Remove image ${idx + 1}?`)) return
    setEditImages(prev => prev.filter((_, i) => i !== idx))
  }

  async function addEditImages(e: React.ChangeEvent<HTMLInputElement>) {
    const files = Array.from(e.target.files || []).filter(f => f.type.match(/image\/(jpeg|png|webp)/))
    if (files.length === 0) return
    show(`Uploading ${files.length} image(s)...`)
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        if (!res.ok || json.error) { show(`Image ${i + 1} failed: ${json.error || 'Upload error'}`); return }
        newUrls.push(json.url)
      } catch (err: any) { show(`Image ${i + 1} failed: ${err.message}`); return }
    }
    setEditImages(prev => [...prev, ...newUrls])
    show(`${files.length} image(s) added!`)
  }
  ...
  async function saveEdit() {
    ...
    const { error } = await supabase.from('gallery').update({
      ...
      reading_mode: editReadingMode,
      image_urls: editImages,
    }).eq('id', id)
    if (error) { show('Failed: ' + error.message); setEditSaving(false); return }
    setItem((p: any) => ({
      ...p,
      title: editTitle.trim(),
      description: editDesc || null,
          ...
          {editingCommentId === c.id ? (
            <div className="mt-1">
              ...
              <div className="flex gap-1.5 mt-1">
                <button onClick={() => editCommentSave(c.id)} className="text-[0.65rem] text-[#c084fc] bg-transparent border-none cursor-pointer">Save</button>
                <button onClick={() => setEditingCommentId(null)} className="text-[0.65rem] text-[#71717a] bg-transparent border-none cursor-pointer">Cancel</button>
              </div>
            </div>
          ) : (
    ...
    <div className="min-h-screen bg-black">
      ...
      <div className="bg-[#09090b]/95 backdrop-blur border-b border-[#27272a] sticky top-0 z-20">
        ...
        <div className="hidden md:flex gap-3 items-start p-4">
          ...
          {/* Right-side action buttons — DELETE REMOVED */}
          <div className="flex flex-col gap-2 shrink-0">
            ...
            {user && item.author_id === user.id && (
              <button onClick={openEditModal} className="flex items-center gap-2 px-4 py-2 rounded-lg border border-[#3f3f46] cursor-pointer text-sm text-[#a1a1aa] hover:border-[#a855f7]">
        ...
        <div className="flex flex-col gap-2 p-3 md:hidden">
          <div className="flex gap-2 items-start">
            ...
            <div className="flex flex-col gap-1 shrink-0">
              ...
              {user && item.author_id === user.id && (
                <button onClick={openEditModal} className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-lg border border-[#3f3f46] cursor-pointer text-[0.6rem] text-[#a1a1aa] hover:border-[#a855f7]">
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/90 z-[110] flex items-center justify-center p-4" onClick={()=>setEditing(false)}>
          <div className="bg-[#18181b] rounded-2xl w-full max-w-[520px] border border-[#27272a] max-h-[90vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            <div className="p-4">
              ...
              <div className="flex flex-col gap-3">
                ...
                <div>
                  ...
                  <input value={editTitle} onChange={e=>setEditTitle(e.target.value)} className="w-full bg-[#27272a] border border-[#3f3f46] rounded-lg p-2 text-[#e4e4e7] text-sm outline-none focus:border-[#a855f7]" />
                </div>

                {/* Description */}
                <div>
                  <label className="block text-xs text-[#71717a] mb-1">Description <span className="text-[#52525b]">(max 80 words)</span></label>
                  <textarea value={editDesc} onChange={e => { const words = e.target.value.trim().split(/\s+/).filter(Boolean); if (words.length <= 80) setEditDesc(e.target.value); else setEditDesc(words.slice(0,80).join(' ')) }} rows={3} className="w-full bg-[#27272a] border border-[#3f3f46] rounded-lg p-2 text-[#e4e4e7] text-sm outline-none resize-y font-[inherit] focus:border-[#a855f7]" />
                  <div className="text-right text-[0.65rem] text-[#52525b] mt-0.5">{editDesc.trim().split(/\s+/).filter(Boolean).length}/80 words</div>
                </div>

                {/* Reading Mode — only for albums */}
                {editImages.length > 1 && (
                  <div>
                    <label className="block text-xs text-[#71717a] mb-1">Reading Mode</label>
                    <div className="flex gap-1.5">
                      <button onClick={() => setEditReadingMode('horizontal')} className={`px-3 py-1.5 rounded-lg cursor-pointer text-xs font-medium border ${editReadingMode === 'horizontal' ? 'border-[#a855f7] text-[#c084fc] bg-purple-500/10' : 'border-[#3f3f46] text-[#71717a] bg-transparent'}`}>◀▶ Horizontal</button>
                      <button onClick={() => setEditReadingMode('webtoon')} className={`px-3 py-1.5 rounded-lg cursor-pointer text-xs font-medium border ${editReadingMode === 'webtoon' ? 'border-[#a855f7] text-[#c084fc] bg-purple-500/10' : 'border-[#3f3f46] text-[#71717a] bg-transparent'}`}>▼ Webtoon</button>
                    </div>
                  </div>
                )}

                {/* Image Management — for albums */}
                {editImages.length > 1 && (
                  <div>
                    <label className="block text-xs text-[#71717a] mb-1.5">Images ({editImages.length})</label>
                    <div className="grid grid-cols-4 md:grid-cols-5 gap-1.5 mb-2">
                      {editImages.map((url, pi) => (
                        <div key={pi} className="relative group">
                          <img src={url} className="w-full aspect-[3/4] object-cover rounded bg-[#27272a]" alt={`Image ${pi + 1}`} />
                          ...
                          <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1 rounded">
                            {pi > 0 && (
                              <button onClick={() => moveEditImage(pi, 'up')}
                            ...
                            <button onClick={() => removeEditImage(pi)}
                            ...
                            {pi < editImages.length - 1 && (
                              <button onClick={() => moveEditImage(pi, 'down')}
                                className="w-5 h-5 bg-[#27272a] border border-[#52525b] rounded text-[0.6rem] text-white cursor-pointer flex items-center justify-center">▶</button>
                            )}
                          </div>
                        </div>
                    ...
                    <button onClick={() => {
                      ...
                      input.multiple = true
                      input.onchange = (e) => addEditImages(e as any)
                      input.click()
                    }}
                ...
                <div className="flex gap-2 mt-1">
                  <button onClick={saveEdit} disabled={editSaving}
                    className="flex-1 py-2 bg-[#7c3aed] text-white rounded-lg text-sm font-medium border-none cursor-pointer hover:bg-[#6d28d9] disabled:opacity-50">
                    {editSaving ? 'Saving...' : 'Save Changes'}
                  </button>
                  <button onClick={deleteGalleryItem}
                    className="flex-1 py-2 border border-[#ef4444] text-[#f87171] bg-transparent rounded-lg cursor-pointer text-sm hover:bg-red-500/10">
                    Delete
                  </button>
                </div>
              </div>
