...
export default function GalleryDetailPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!c && data) {
        ...
        // Load comments
        const { data: cmts } = await supabase.from('gallery_comments')
          .select('*, profiles(display_name, handle, avatar_url)')
          .eq('gallery_id', id).order('created_at', { ascending: false })
        if (!c) setComments(cmts ?? [])

        // Check like
        if (user) {
          const { data: lk } = await supabase.from('gallery_likes')
            .select('*').eq('user_id', user.id).eq('gallery_id', id).maybeSingle()
          if (!c) setLiked(!!lk)
        }
      }
  ...
  async function toggleLike() {
    if (!user || !item) { show('Sign in to like!'); return }
    if (liked) {
      await supabase.from('gallery_likes').delete().eq('user_id', user.id).eq('gallery_id', id)
      setItem((p: any) => ({ ...p, total_likes: Math.max(0, (p.total_likes ?? 1) - 1) }))
      setLiked(false); show('Like removed')
    } else {
      await supabase.from('gallery_likes').insert({ user_id: user.id, gallery_id: id })
      setItem((p: any) => ({ ...p, total_likes: (p.total_likes ?? 0) + 1 }))
      setLiked(true); show('Liked!')
    }
  }

  async function postComment() {
    if (!user || !item) { show('Sign in to comment!'); return }
    if (!commentText.trim()) { show('Write something first!'); return }
    const { data, error } = await supabase.from('gallery_comments').insert({
      user_id: user.id, gallery_id: id, body: commentText.trim()
    }).select('*, profiles(display_name, handle, avatar_url)').single()
    if (error) { show('Failed: ' + error.message); return }
    if (data) { setComments(prev => [data, ...prev]); setCommentText(''); show('Comment posted!') }
  }

  if (loading) return <div className="min-h-screen flex items-center justify-center text-[#52525b]">Loading...</div>
  if (!item) return <div className="min-h-screen flex items-center justify-center text-[#71717a]">Artwork not found</div>

  const images = item.image_urls || []
  const readingMode = item.reading_mode || 'horizontal'
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <div className="bg-[#09090b]/95 backdrop-blur border-b border-[#27272a] sticky top-0 z-20">
        <div className="flex gap-2 items-center p-3 md:p-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {item.description && <div className="text-xs text-[#a1a1aa] mt-1 line-clamp-1">{item.description}</div>}
            <div className="flex items-center gap-3 mt-1">
              ...
              {images.length > 1 && <span className="text-[0.68rem] text-[#71717a]">{images.length} images</span>}
            </div>
          </div>
          <div className="flex flex-col gap-1.5 shrink-0">
            <button onClick={toggleLike} className={`flex items-center gap-1 px-2.5 py-1 rounded-lg border cursor-pointer text-xs ${liked ? 'bg-red-500/15 border-red-500/40 text-[#f87171]' : 'border-[#3f3f46] text-[#a1a1aa]'}`}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill={liked ? '#f87171' : 'none'} stroke="currentColor" strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
              {liked ? 'Liked' : 'Like'}
            </button>
            <button onClick={() => setShowShare({ title: item.title, url: `${typeof window !== 'undefined' ? window.location.origin : ''}/gallery/${id}` })}
              className="flex items-center gap-1 px-2 py-1 rounded-lg border border-[#3f3f46] cursor-pointer text-xs text-[#a1a1aa]">Share</button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-[800px] mx-auto p-1.5 md:p-3">
        {readingMode === 'webtoon' ? (
          ...
          images.map((url: string, i: number) => (
            <img key={i} src={url} className="w-full" style={{ marginTop: i > 0 ? '-4px' : '0' }} alt={`Image ${i+1}`} />
          ))
        ) : (
          // Horizontal pages
          <div className="relative">
            {images.length > 0 && (
              <img src={images[currentPage]} className="w-full rounded-lg" alt={`Image ${currentPage+1}`} />
            )}
            {images.length > 1 && (
              <div className="flex items-center justify-center gap-3 mt-3">
                <button onClick={() => setCurrentPage(p => Math.max(0, p - 1))} disabled={currentPage === 0}
                  className={`px-4 py-2 border rounded-lg text-sm cursor-pointer ${currentPage === 0 ? 'border-[#27272a] text-[#3f3f46]' : 'border-[#3f3f46] text-[#a1a1aa] hover:border-[#a855f7]'} bg-transparent`}>
                  ◀ Prev
                </button>
                <span className="text-sm text-[#71717a]">{currentPage + 1} / {images.length}</span>
                <button onClick={() => setCurrentPage(p => Math.min(images.length - 1, p + 1))} disabled={currentPage === images.length - 1}
                  className={`px-4 py-2 border rounded-lg text-sm cursor-pointer ${currentPage === images.length - 1 ? 'border-[#27272a] text-[#3f3f46]' : 'border-[#3f3f46] text-[#a1a1aa] hover:border-[#a855f7]'} bg-transparent`}>
                  Next ▶
                </button>
              </div>
            )}
      ...
      <div className="max-w-[800px] mx-auto mt-7 bg-[#18181b] rounded-2xl p-4 mb-8 mx-3">
        ...
        {comments.length === 0 && <p className="text-[#52525b] text-xs mb-3">No comments yet. Be the first!</p>}
        {comments.map(c => (
        ...
        <div className="flex justify-end mt-2">
          <button onClick={postComment} className="px-4 py-2 bg-[#7c3aed] text-white rounded-xl cursor-pointer text-sm font-medium border-none hover:bg-[#6d28d9]">Post Comment</button>
        </div>
      </div>
