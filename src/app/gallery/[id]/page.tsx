          ...
          <div className="bg-[#18181b] rounded-2xl w-full max-w-[420px] border border-[#27272a] p-4" onClick={e=>e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex gap-2">
                ...
                }} className="flex-1 py-2 bg-[#7c3aed] text-white rounded-lg text-sm font-medium border-none cursor-pointer hover:bg-[#6d28d9]">Save Changes</button>
                <button onClick={async()=>{
                  if(!confirm('Delete this gallery item?'))return
                  if(!confirm('This cannot be undone. Are you sure?'))return
                  const {error}=await supabase.from('gallery').delete().eq('id',id)
                  if(error){show('Delete failed: '+error.message);return}
                  show('Deleted');window.location.href='/gallery'
                }} className="flex-1 py-2 border border-[#ef4444] text-[#f87171] bg-transparent rounded-lg text-sm cursor-pointer hover:bg-red-500/10">Delete</button>
              </div>
            </div>
          </div>
        </div>
