...
export default function GalleryDetailPage() {
    ...
    async function load() {
      try {
        ...
        if (!cancelled && data) {
          ...
          // Increment view count (non-blocking, silent)
          anonDb
            .from('gallery')
            ...
            .catch(() => {}) // ignore errors on view counter
  ...
  // Like / Favorite / Follow / Comment functions remain the same
            ...
            {imgs.length > 1 && (
              <div className="flex items-center justify-center gap-3 mt-3">
                <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0}
                  className={`px-4 py-2 border rounded-xl text-sm cursor-pointer bg-transparent ${page === 0 ? 'border-[#27272a] text-[#3f3f46]' : 'border-[#3f3f46] text-[#a1a1aa] hover:border-[#a855f7]'}`}>
                  ◀ Prev
                </button>
                ...
                <button onClick={() => setPage(p => Math.min(imgs.length - 1, p + 1))} disabled={page === imgs.length - 1}
                  className={`px-4 py-2 border rounded-xl text-sm cursor-pointer bg-transparent ${page === imgs.length - 1 ? 'border-[#27272a] text-[#3f3f46]' : 'border-[#3f3f46] text-[#a1a1aa] hover:border-[#a855f7]'}`}>
                  Next ▶
                </button>
        ...
        {comments.map(c => (
          <div key={c.id} className="flex gap-2 mb-3">
            {c.profiles?.avatar_url ? (
              <img src={c.profiles.avatar_url} className="w-7 h-7 rounded-full object-cover shrink-0" alt="" />
            ) : (
              <Avatar name={c.profiles?.display_name || 'User'} size={28} />
            )}
            <div>
