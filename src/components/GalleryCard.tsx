  ...
  return (
    <Link href={`/gallery/${item.id}`} className="group cursor-pointer block no-underline">
      <div className="relative rounded-xl overflow-hidden bg-[#27272a]">
        ...
        {item.is_mature && (
          <div className="absolute top-1.5 left-1.5 bg-amber-600 text-white text-[0.6rem] px-1.5 py-0.5 rounded font-bold">MATURE</div>
        )}
        {imgs.length > 1 && (
          <div className="absolute bottom-1.5 left-1.5 bg-black/70 text-[0.59rem] px-1.5 py-0.5 rounded text-[#a1a1aa]">ALBUM</div>
        )}
      </div>
