...
export function UploadModal({ onClose, onToast }: { onClose: () => void; onToast: (m: string) => void }) {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }: any) => {
        ...
        if (error) console.error('Error loading series:', error)
      })
  }, [user, supabase])
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .limit(1)
      .then(({ data }: any) =>
        setChapterNumber(data && data.length > 0 ? data[0].chapter_number + 1 : 1)
      )
    ...
    try {
      ...
      if (mode === 'new') {
        ...
        if (thumbFile) {
          ...
          if (thumbErr) {
            // Non-fatal: warn but continue without thumbnail
        ...
        const { data: ns, error: seriesErr } = await supabase
          .from('series')
        ...
        {step === 'existing' && (
          <div className="p-4">
            ...
            {loadingSeries ? (
              ...
              <p className="text-[#52525b] text-xs py-4 text-center">You have no series yet. Create one first!</p>
            ) : (
        ...
        {step === 'form' && (
          <>
            <div className="p-4 flex flex-col gap-3">
              ...
              {/* Error display */}
              {uploadError && (
                <div className="bg-red-500/10 border border-red-500/30 text-red-400 text-xs rounded-xl p-2.5">
                  {uploadError}
                </div>
              )}
              ...
              {/* Genres + Description — new series only */}
              {mode === 'new' && (
                <div className="border-t border-[#27272a] pt-3">
                  <label className="block text-xs text-[#71717a] mb-1.5">
                    Genres <span className="text-[#52525b]">(pick up to 3)</span>
                  </label>
                    ...
                    {GENRES_ALL.map(g => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          else if (n.size < 3) n.add(g)
                          else return
                          setGenres(n)
                        }}
                        className={`px-2.5 py-1 rounded-full text-[0.73rem] cursor-pointer border ${
                          genres.has(g)
                            ? 'border-[#a855f7] text-[#c084fc] bg-purple-500/10'
                            : genres.size >= 3
                            ? 'border-[#27272a] text-[#3f3f46] bg-transparent cursor-not-allowed'
                            : 'border-[#3f3f46] text-[#71717a] bg-transparent'
                        }`}
                      >
                        {g}
                      </button>
              ...
              <div>
                <label className="block text-xs text-[#71717a] mb-1.5">
                  Chapter Pages <span className="text-[#52525b]">(JPG, PNG, WebP — max 20MB each)</span>
                </label>
                <div
                      ...
                      {files.map((f, i) => (
                        <div key={i} className="flex items-center gap-2 py-1 border-b border-[#27272a] text-xs">
                          ...
                          <button
                            ...
                            className="text-[#71717a] hover:text-[#f87171] bg-transparent border-none cursor-pointer text-xs px-1"
                          >
                            ✕
                          </button>
                        </div>
              ...
              <button onClick={() => setStep('choose')} className="bg-transparent border-none text-[#71717a] cursor-pointer text-sm text-left">
                ← Back
              </button>
            </div>
            ...
            <div className="p-3 border-t border-[#27272a] flex gap-2">
              <button
                onClick={onClose}
                className="flex-1 py-2 border border-[#3f3f46] rounded-lg bg-transparent text-[#a1a1aa] cursor-pointer text-sm"
              >
                Cancel
              </button>
              <button
